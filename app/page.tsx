...
export default function Page() {
  ...
  const t = useMemo(() => {
    const sv = {
      ...
      copied: 'Kopierad!',
      mintHint: 'Vill du prova on-chain? I MVP:en kan vi mynta en CreatorPass (Solana) till din wallet efter betalning.',
    ...
    const en = {
      ...
      heroSub: 'A simple CreatorPass that unlocks process, sketches and hi-res for your fans.',
      ...
      unlockedSub: 'See the process steps below and download the files. (Demo mode)',
      downloadAll: 'Download all (.zip) – demo',
      copyLink: 'Copy magic link',
      copied: 'Copied!',
      mintHint: 'Want to try on-chain? In the MVP we can mint a CreatorPass (Solana) to your wallet after payment.',
  ...
  // Stabil placeholders (picsum.photos)
  const demoImages = [
  ...
  const copyMagic = async () => {
    try {
      await navigator.clipboard.writeText(`https://creatorpass.one/demo?email=${encodeURIComponent(email)}`);
      setCopied(true);
      ...
      <div className="sticky top-0 z-40 backdrop-blur bg-neutral-950/70 border-b border-neutral-800">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-neutral-400 text-sm ml-3">MVP Demo</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <a href="#how" className="text-sm px-3 py-1.5 rounded-full border border-neutral-700 hover:bg-neutral-800">
              {t.ctaSecondary}
            </a>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep('checkout')} className="px-5 py-3 rounded-2xl bg-white text-neutral-900 font-semibold hover:opacity-90">
                {t.ctaPrimary}
              </button>
              <a href="#pack" className="px-5 py-3 rounded-2xl border border-neutral-700 hover:bg-neutral-900">
                {t.ctaSecondary}
              </a>
            </div>
          </div>
          <div className="relative">
            <div className="grid grid-cols-3 gap-2">
              {demoImages.map((img, i) => (
                <img key={i} src={img.thumb} alt={img.caption} className="rounded-xl object-cover h-32 w-full" />
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="shrink-0 w-full md:w-auto">
              <div className="rounded-2xl border border-neutral-800 p-5 text-center">
                ...
                <button onClick={() => setStep('checkout')} className="w-full px-5 py-3 rounded-xl bg-white text-neutral-900 font-semibold hover:opacity-90">
                  {t.unlockNow}
                </button>
              </div>
            </div>
          ...
          {step === 'checkout' && (
            <div className="mt-8 rounded-2xl border border-neutral-800 p-6 bg-neutral-950">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setStep('landing')} className="text-sm text-neutral-400 hover:text-neutral-200">
                  {t.back}
                </button>
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="mt-4 flex items-start gap-2">
                    <input id="tos" type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} className="mt-1"/>
                    <label htmlFor="tos" className="text-sm text-neutral-400">{t.tosLabel}</label>
                  </div>
                  <button onClick={async ()=>{const r=await fetch('/api/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})}); const j=await r.json().catch(()=>null); if(j?.url){window.location.href=j.url;} else {alert('Checkout failed')}}} disabled={!email || !agree}
                    className="mt-5 w-full px-5 py-3 rounded-xl bg-white text-neutral-900 font-semibold disabled:opacity-40">
                    {t.payBtn(priceSEK)}
                  </button>
                  <p className="text-xs text-neutral-500 mt-3">Demo-läge: kortuppgifter krävs ej.</p>
                </div>
                <div>
          ...
          {step === 'unlocked' && (
            <div className="mt-8 rounded-2xl border border-neutral-800 p-6 bg-neutral-950">
              ...
              <div className="mt-6 flex flex-col md:flex-row gap-3">
                <button className="px-4 py-3 rounded-xl border border-neutral-700 hover:bg-neutral-900">
                  {t.downloadAll}
                </button>
                <button onClick={async ()=>{await navigator.clipboard.writeText(`https://creatorpass.one/demo?email=${encodeURIComponent(email)}`);}}
        ...
        <ol className="space-y-3">
          {t.howSteps.map((s, i) => (
            <li key={i} className="flex gap-3 items-start">
              <div className="w-7 h-7 rounded-full bg-white text-neutral-900 font-bold flex items-center justify-center">{i + 1}</div>
